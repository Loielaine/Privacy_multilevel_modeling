...
data_complete<-read.csv2(paste0(dir,'demo_bidr_complete_pmm.csv'),sep =',')[,6:24]
summary(data_complete)
data_complete<-apply(data_complete,2,as.numeric)
apply(data_complete,2,mean)
...
#2018.11.22 transformation
#using loadings in CFA 2 factors as weights
loading <- read.csv2(paste0(dir,'loadings_polr.csv'),sep =',')[1:19,2:3]
loading <-apply(loading,2,as.numeric)
score<-data_complete %*% loading
summary(score)
sd(score[,1])
sd(score[,2])
